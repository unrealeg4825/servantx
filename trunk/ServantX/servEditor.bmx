...
Global scaleXInit# = 0.0
Global scaleYInit# = 0.0
Global scaleZInit# = 0.0
Global deltaX#       = 1.0
Global deltaY#       = 1.0
Global deltaZ#       = 1.0
...
'Window
	...
	m_nodes = alAddMenuItem(menu, "Tools/Nodes");
...
Select event.type_
	...
	Case ALEVENT_CLICK
		Select event.object_
			...
			Case m_nodes
				If alCheckExistence(nwin) = False Then
					Create_Node_Props()
				EndIf			
			...
			Case dropnode
				...
				n.y# = xEntityy(camera)
				n.z# = xEntityz(camera)
				n.sprite = xCreateCube()
				xPositionEntity n.sprite,n.x,n.y,n.z
				xEntityPickMode n.sprite,2
			Rem
			Case scalenowbutton
	...
	If pentity <> 0 Then
		...
		Select controlType
			...
			Case 2
				...
				used_controller$ = "Scale: "+scaleXInit+" "+scaleYInit+" "+scaleZInit
			Default
				'used_controller$ = ""
			End Select
			...
			If xMouseDown(1) And selectMask <> 0
				...
				useZ     = selectMask & 4
				useG     = selectMask & 8
				factorX# = 0.1 / Float(xGraphicsWidth())
				factorY# = 0.1 / Float(xGraphicsHeight())
				Select controlType
				...
				Case 2
					...
					' x-axis
					If useX
						move#      = ComputeMove(camera, 1.0, 0.0, 0.0) * factorX * dist
						deltaX     = deltaX     + move
					...
					' y-axis
					If useY
						move#      = ComputeMove(camera, 0.0, 1.0, 0.0) * factorY * dist
						deltaY     = deltaY     + move
					...
					' z-axis
					If useZ
						move#      = ComputeMove(camera, 0.0, 0.0, 1.0) * factorX * dist
						deltaZ     = deltaZ     + move
...
Function Save_zone(xmlfilename$)
	...
	For b:Building = EachIn Buildings
		...
		bnode.Attribute("xscale").value = xEntityScaleX(b.entity)
		bnode.Attribute("yscale").value = xEntityScaleY(b.entity)
		bnode.Attribute("zscale").value = xEntityScaleZ(b.entity)
	...
	Local Nnode:xmlNode = DocRoot.AddNode("NODES")
	
	For Local n:node = EachIn nodes
	
		Local nod:xmlNode = Nnode.AddNode("ND")
		nod.Attribute("xloc").value = xEntityX(n.sprite)
		nod.Attribute("yloc").value = xEntityY(n.sprite)
		nod.Attribute("zloc").value = xEntityZ(n.sprite)		
...
Function Create_Node_Props()

	nwin = alCreateGWindow("Nodes", 200,200, 240, 138, AL_WinbuttonClose);
	alSetSAreaPadding(nwin, 10, 10, 10, 10);
	dropnode  = alCreateGButton(nwin, "Create Node", 10, 10, 150, -1);
